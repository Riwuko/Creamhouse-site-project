...
from cosmetics.models import Cosmetic, BodyCosmetic, FaceCosmetic, FeetCosmetic, HairCosmetic, HandsCosmetic, Ingredient, IngredientName, CosmeticProperties
from cosmetics.validation import  validate_cosmetic_json, validate_ingredients_json
...
class AddNewCosmetic(View):
    ...
    def post(self, request, requested_cosmetics):
    	...
    	ingredients = requested_cosmetics['commonCosmeticData'].pop('ingredients')
    	...
    	for propertyName in properties:
    		obj,created = CosmeticProperties.objects.get_or_create(name=propertyName)
    	...
    	if typ=="body":
    		cosmetic = BodyCosmetic(name=name,target_gender=target_gender,**requested_cosmetics)
    	elif typ=="hands":
    		cosmetic = HandsCosmetic(name=name,target_gender=target_gender,**requested_cosmetics)
    	elif typ=="feet":
    		cosmetic = FeetCosmetic(name=name,target_gender=target_gender,**requested_cosmetics)
    	elif typ=="hair":
    		cosmetic = HairCosmetic(name=name,target_gender=target_gender,**requested_cosmetics)
    	elif typ=="face":
    		cosmetic = FaceCosmetic(name=name,target_gender=target_gender,**requested_cosmetics)
    	else:
    		return JsonResponse({'message': ':('})
