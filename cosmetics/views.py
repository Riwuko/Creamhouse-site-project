...
class AddNewIngredient(View):
    ...
    def post(self,request, requested_ingredients):
    	for i in range (len(requested_ingredients)):
    		if (i==0):
    			main_ingredient = Ingredients.objects.create(main_name=requested_ingredients[i]["name"])
    		else:
    			IngredientNames.objects.create(main_ingredient, name=requested_ingredients[i]["name"])
